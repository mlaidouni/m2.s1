...
def parse_memory_operand(s):
    ...
    """
    # formats accepted: "offset($base)", "($base)", "$reg" (rare)
    s = s.strip()
...
def encodage_b(opcode, r1, r2, aux):
    ...
    Returns:
        int: instruction encodée
    """
    # FIXME: 0xFFF pour garder 12 bits
    return (opcode << 24) | (r1 << 18) | (r2 << 12) | (aux & 0xFFF)
...
def build_instr_btype_load_store(mnemonic, ops, opcode):
    """Construit une ligne de type B load/store (2 ou 3 opérandes).
    ...
    if len(ops) == 2:
        ...
        aux = offset
    elif len(ops) == 3:  # FIXME: Version alternative ?
        # alternative rd, base, offset
        r1 = parse_register(ops[0])
        r2 = parse_register(ops[1])
        aux = parse_imm(ops[2])
    else:
        raise ValueError(f"{mnemonic} attend 2 ou 3 opérandes")
...
def assemble_line(line, pc, labels):
    ...
    if mnemonic in ("add", "sub", "mul", "div", "and", "or", "xor"):
        return assemble_line_type_a(mnemonic, ops, opcode)

    # load/store: lw rd, offset(base)  OR  sw rs, offset(base)
    if mnemonic in ("lw", "sw"):
        return assemble_line_type_load_store(mnemonic, ops, opcode)
...
def assemble(src_path, out_path):
    ...
    # write binary (big-endian 4 bytes per instruction)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python assembler.py program.asm out.bin")
        sys.exit(1)
